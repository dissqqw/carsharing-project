...
const LinkEnumModal = ({ open, onClose, onSuccess, classId, className }) => {
  ...
  const handleSubmit = async () => {
    if (!enumId) { setError('Выберите справочник'); return; }
    setLoading(true);
    setError('');
    try {
      await fetch(`http://127.0.0.1:5000/api/class/${classId}/enum/link`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id_enum: enumId, is_required: isRequired }),
      });
      onSuccess();
      onClose();
    } catch (e) {
      setError(typeof e === 'string' ? e : 'Ошибка при привязке справочника');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={onClose}>
      <div style={{ width: 733, background: '#FFFFFF', borderRadius: 10, padding: '40px', position: 'relative', fontFamily: '"Inter Tight", sans-serif' }} onClick={(e) => e.stopPropagation()}>
        ...
        <h3 style={{ fontWeight: 600, fontSize: 26, lineHeight: '28px', color: '#1C1C19', margin: '0 0 12px 0' }}>Привязать справочник</h3>
        <p style={{ fontWeight: 400, fontSize: 18, lineHeight: '20px', color: '#707070', margin: '0 0 24px 0' }}>
          Добавление справочника для класса «{className}»
        </p>

        <div style={{ display: 'flex', gap: 20, marginBottom: 24 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            <div onClick={() => { setEnumDropdownOpen(!enumDropdownOpen); setRequiredDropdownOpen(false); }} style={{ ...fieldStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
              <span style={{ color: selectedEnum ? '#1C1C19' : '#707070' }}>{selectedEnum ? selectedEnum.name : 'Справочник *'}</span>
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none" style={{ transform: enumDropdownOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
              ...
              <div style={{ position: 'absolute', top: 54, left: 0, right: 0, background: '#FFFFFF', border: '1px solid #8D8D8B', borderRadius: 10, maxHeight: 250, overflow: 'auto', zIndex: 10 }}>
                {enums.map((e) => (
                  <div key={e.id_enum} className={`dropdown-option ${enumId === e.id_enum ? 'dropdown-option--selected' : ''}`}
                    onClick={() => { setEnumId(e.id_enum); setEnumDropdownOpen(false); }}>
                    {e.name}
                  </div>
                ))}
              </div>
          ...
          <div style={{ flex: 1, position: 'relative' }}>
            <div onClick={() => { setRequiredDropdownOpen(!requiredDropdownOpen); setEnumDropdownOpen(false); }} style={{ ...fieldStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
